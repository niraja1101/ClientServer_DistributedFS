...
class SimpleHT:
  ...
  def put(self, key, value):
    ...
    self.files[key.data] = pickle.loads(value.data)
    return True

# Retrieve something from the HT
  def getdirect(self, key):
    # Default return value
    rv = ''
    # If the key is in the data structure, return properly formated results
    key = key.data
    if key in self.directory:
      rv = self.directory[key]
    return Binary(pickle.dumps(rv))

  # Insert something into the HT
  def putdirect(self, key, value):
    # Remove expired entries
    self.directory[key.data] =pickle.loads(value.data)
    return True
# Retrieve something from the HT
  ...
  # Insert something into the HT
  def putwholefile(self,value):
    # Remove expired entries
    self.files =pickle.loads(value.data)
    return True



# Retrieve something from the HT
  def getwholedirect(self):
    # Default return value
    rv = self.directory
    return Binary(pickle.dumps(rv))

  # Insert something into the HT
  def putwholedirect(self,value):
    # Remove expired entries
    self.directory =pickle.loads(value.data)
    return True
...
def main():
  port=51234
  if(len(argv)<2):
     print('usage: %s <metaserver_port>' % argv[0])
     exit(1)
  port=int(sys.argv[1])
...
def serve(port):
  ...
  file_server.register_function(sht.getdirect)
  file_server.register_function(sht.putdirect)
  file_server.register_function(sht.getwholedirect)
  file_server.register_function(sht.putwholedirect)
  file_server.register_function(sht.getwholefile)
  file_server.register_function(sht.putwholefile)
